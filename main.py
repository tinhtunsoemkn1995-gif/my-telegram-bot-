...
# သင်ပေးထားသော Bot Token နှင့် Admin ID ကို ထည့်သွင်းထားပါသည်
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8566855727:AAHcStHg78D8fLW3GWSe4JeQfIZkGzzyWUw')
ADMIN_ID = os.environ.get('ADMIN_ID', '6102959038')
...
async def genkey(message):
    ...
    try:
        ...
        if result:
            ...
            await bot.reply_to(message, "❌ File သိမ်းဆည်းရာတွင် အမှားအယွင်းဖြစ်ပေါ်ခဲ့ပါသည်။")
    except Exception as e:
...
async def handle_input(message):
    ...
    if message.chat.id in user_data:
        ...
        if await check_session_url(session_url=url):
            ...
            await bot.reply_to(message, "Session URL အားသိမ်းဆည်းပြီးပါပြီ။ /scan 6, 7, 8, 9, all, ascii-lower စသည်ဖြင့် ရွေးချယ်စတင်နိုင်ပါပြီ။")
        else:
...
async def periodic_update_scheduler():
    global SUCCESS_CODE
    while True:
...
def Minute_to_Hour(total_minutes):
    ...
    return f"{hours}h {minutes}m" if hours > 0 and minutes > 0 else (f"{hours}h" if hours > 0 else f"{minutes}m")
...
async def main():
    ...
    try:
        ...
        asyncio.create_task(periodic_update_scheduler())
        await start_polling()
    finally:
        await session.close()
